...
//new tail fit temperature parameters
Real T0_tail, Tv0_tail, Tvc_tail, Tref_tail;
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  // new tail-fit temperature parameters ( T0, Tv0, Tvc, Tref)
  T0_tail   = pin->GetOrAddReal("problem","T0_tail",1.30e7);
  Tv0_tail  = pin->GetOrAddReal("problem","Tv0_tail",1.39e9);
  Tvc_tail  = pin->GetOrAddReal("problem","Tvc_tail",7.53e8);
  Tref_tail = pin->GetOrAddReal("problem","Tref_tail",5.0);
...
void SNInnerX1(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim, FaceField &b,
  ...
  //pres = gasEntropy * std::pow(rho, gammagas);
    ...
    // temperature profile at reference time Tref_tail
  Real Texp = -(vel - Tv0_tail) / Tvc_tail;
  if (vel < Tv0_tail) Texp = 0.0;  // flat core at T(Tv0_tail)

  Real T_ref_v = T0_tail * std::exp(Texp);

  // adiabatic time scaling for gamma 
  Real temp_pow = 3.0 * (gammagas - 1.0);
  Real temp = T_ref_v * std::pow(Tref_tail / t_phys, temp_pow);

  // pressure from your custom EOS form
  pres = PressureFromTRho(temp, rho);
