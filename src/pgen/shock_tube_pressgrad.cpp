...
void mySourceTerm(MeshBlock* pmb, const Real time, const Real dt,
...
void Mesh::InitUserMeshData(ParameterInput* pin) {
  EnrollUserExplicitSourceFunction(mySourceTerm);
  return;
}
...
void mySourceTerm(MeshBlock* pmb, const Real time, const Real dt,
  ...
  Real grad1, grad2, grad3;
  for (int k = pmb->ks; k <= pmb->ke; ++k) {
    for (int j = pmb->js; j <= pmb->je; ++j) {
      for (int i = pmb->is; i <= pmb->ie; ++i) {
        grad1 = pmb->pcoord->pressureGradients(0,k,j,i);
        grad2 = pmb->pcoord->pressureGradients(1,k,j,i);
        grad3 = pmb->pcoord->pressureGradients(2,k,j,i);
        printf("gradients: %5.3e %5.3e %5.3e\n",grad1,grad2,grad3);
      }
    }
  }
}
