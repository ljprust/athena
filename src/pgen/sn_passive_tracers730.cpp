    ...
    for (int j=js; j<=je; ++j) {
      for (int i=is; i<=ie; ++i) {
        ...
        //using velocity ranges from Boos 2021, https://arxiv.org/pdf/2101.12330, figure 7 assuming Mtot ~ 0.9 Msun
            if (v <= 0.2 * std::pow(10, 9)){
                    pscalars->r(0, k, j, i) = 1.0;
            }
             else if (0.2 * std::pow(10, 9) < v && v <= 0.55 * std::pow(10, 9)){
                    pscalars->r(1, k, j, i) = 1.0;
            }
             else if (0.55 * std::pow(10, 9) < v && v <= 0.75 * std::pow(10, 9)){
                    pscalars->r(2, k, j, i) = 1.0;
            }
             else if ( v > 0.75 * std::pow(10, 9) ){
  ...
  for (int k=kl; k<=ku; ++k) {
    for (int j=jl; j<=ju; ++j) {
      for (int i=1;  i<=ngh; ++i) {
        ...
        if (vel <= 0.2 * std::pow(10, 9)) {
            prim_scalar(0,k,j,il-i) = 1.0;
        }
         else if (0.2 * std::pow(10, 9) < vel && vel <= 0.55 * std::pow(10, 9)){
             prim_scalar(1,k,j,il-i) = 1.0;
         }
         else if (0.55 * std::pow(10, 9) < vel && vel <= 0.75 * std::pow(10, 9) ){
             prim_scalar(2,k,j,il-i) = 1.0;
         }
         else if ( vel > 0.75 * std::pow(10, 9) ){
