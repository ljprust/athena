...
Real r, t0, v0, v, rho, pres, prefactor;

t0 = Rej/vmax;
v0 = std::sqrt(Eej / (6.0 * Mej));
prefactor = Mej / (8 * 3.14159 * std::pow(v0 * t0, 3));
...
void SNInnerX1(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim, FaceField &b,
  ...
  Real rho, vel, v0, pres, prefactor, t0;
  ...
  prefactor = Mej / (8 * 3.14159 * std::pow(v0 * (time + t0), 3));
  //rho = prefactor*std::exp(-vel*vel/v0sq)*std::pow(time+t0,-3.0);
  rho = prefactor*std::exp(-vel/v0);
  pres = gasEntropy * std::pow(rho,1.6666666666667);
