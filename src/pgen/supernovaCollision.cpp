...
void MeshBlock::ProblemGenerator(ParameterInput* pin) {
    ...
    Real r, t0, v0sq, v, rho, pres, temp;
    t0 = Rej / vmax;
    ...
    for (int k = ks; k <= ke; ++k) {
        for (int j = js; j <= je; ++j) {
            for (int i = is; i <= ie; ++i) {
                ...
                if (r > Rej) {
                    // ambient medium
                    temp = calcTemperaturePressurePgen(rho0, p0);
                    phydro->u(IDN, k, j, i) = rho0;
                    phydro->u(IM1, k, j, i) = rho0*vel0;
                    phydro->u(IEN, k, j, i) = energyEqPgen(temp, rho0) + 0.5*rho0*vel0*vel0;
                    ...
                    //pres = 0.69e14*std::pow(rho,1.66666666666667);
                    pres = 0.7e14*std::pow(rho,1.66666666666667);
...
void SNInnerX1(MeshBlock* pmb, Coordinates* pco, AthenaArray<Real>& prim, FaceField& b,
    ...
    Real rho, vel, v0sq, pres, prefactor, t0, temp;
    t0 = Rej / vmax;
    v0sq = 4.0 / 3.0 * Eej / Mej;
    ...
    for (int k = kl; k <= ku; ++k) {
        for (int j = jl; j <= ju; ++j) {
            for (int i = 1; i <= ngh; ++i) {
                ...
                prim(IEN, k, j, il - i) = pres; //energyEqPgen(temp, rho);
            }
        }
    }
}
